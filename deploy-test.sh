...
# Update gateway.yml with test domain and staging issuer annotation
...
sed -i.tmp "s/cert-manager.io\/cluster-issuer: letsencrypt-prod/cert-manager.io\/cluster-issuer: letsencrypt-staging/" k8s/gateway.yml
...
if is_step_completed 10; then
    ...
    echo ""
    echo -e "  ${GREEN}Type:${NC}  A"
    echo -e "  ${GREEN}Name:${NC}  test.aicloudops.in"
...
echo -e "   ${GREEN}Full cleanup:${NC}     ./cleanup.sh ${YELLOW}(recommended - removes all resources & checkpoints)${NC}"
echo -e "   ${GREEN}Quick cleanup:${NC}    kubectl delete certificate,gateway,httproute -n bankapp --all"
echo -e "   ${GREEN}Reset only:${NC}       rm $CHECKPOINT_FILE"
